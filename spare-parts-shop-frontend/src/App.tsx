...
const RoleGuard = ({
  ...
  if (role === 'SUPER_ADMIN' || role === 'SUPER_MANAGER') return <>{children}</>;
  if (!allowedRoles.includes(role)) return <Navigate to="/dashboard" replace />;

  if (role === 'EMPLOYEE' && requiredPermission) {
    const perms = user.permissions || [];
    if (!perms.includes(requiredPermission)) return <Navigate to="/dashboard" replace />;
  }
  return <>{children}</>;
};
  ...
  return (
    <>
      ...
      <Routes>
        ...
        {/* Protected Staff Routes — all rendered inside Layout */}
        <Route
          path="/*"
          element={
            <ProtectedRoute>
